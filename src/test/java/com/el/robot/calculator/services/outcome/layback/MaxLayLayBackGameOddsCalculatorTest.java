...
import com.el.robot.calculator.services.impl.outcome.layback.MaxLayOddsShareCalculator;
...
public class MaxLayLayBackGameOddsCalculatorTest extends SpringJUnitTest {
    ...
    @Autowired
    private MaxLayOddsShareCalculator oddsShareCalculator;
    ...
    public void testHigherBack() {
        ...
        assertEquals(groupShare.getBackBetShare().multiply(backBetOption.getPrice().subtract(BigDecimal.ONE)).doubleValue(), groupShare.getLayBetShare().doubleValue(), DELTA);
        assertTrue(groupShare.getLayUserBetShare().subtract(groupShare.getBackBetShare()).doubleValue() > 0);
